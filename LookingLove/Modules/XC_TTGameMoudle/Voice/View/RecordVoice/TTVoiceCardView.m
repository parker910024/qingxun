...
@implementation TTVoiceCardView
...
- (void)onPanGes:(UIPanGestureRecognizer *)ges{
    ...
    if (UIGestureRecognizerStateChanged == ges.state) {
        ...
        self.direction = translation.x > 0 ? TTVoiceCardViewSwipeDirectionRight : TTVoiceCardViewSwipeDirectionLeft;
...
//        self.selectedView.transform = CGAffineTransformMakeRotation(self.direction * self.progress * self.maxRadius);
    ...
    if (UIGestureRecognizerStateEnded == ges.state) {
        ...
        //判断是否需要恢复
        BOOL isNeedResumeCardItem = TTVoiceCardViewSwipeDirectionNone != self.direction && self.progress < 1;
        
        if (isNeedResumeCardItem) {
            self.direction = TTVoiceCardViewSwipeDirectionNone;
        }
...
- (void)continueCardItemAnimation{
    switch (self.direction) {
        case TTVoiceCardViewSwipeDirectionNone:
            [self resumeCardItems];
            break;
        case TTVoiceCardViewSwipeDirectionLeft:
            [self swipeCardItemToDirection:TTVoiceCardViewSwipeDirectionLeft];
            break;
        case TTVoiceCardViewSwipeDirectionRight:
            [self swipeCardItemToDirection:TTVoiceCardViewSwipeDirectionRight];
            break;
    }
}
    ...
    [self.itemViews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (self.selectedView == obj) {
            ...
            if (idx == 0) {
                ...
                if (self.progress) {
                   obj.alpha = self.progress* 0.2 * (idx + 1);
                }else{
                     obj.alpha = 1 - 0.1 * (2 * (idx + 1)+ 1);
                }
...
- (__kindof UIView *)dequeueReuseableCardItemViewWithIdentifier:(NSString *)identifier{
    ...
    [itemView setValue:@(NO) forKey:@"isShowReload"];
    if (!itemView) {
