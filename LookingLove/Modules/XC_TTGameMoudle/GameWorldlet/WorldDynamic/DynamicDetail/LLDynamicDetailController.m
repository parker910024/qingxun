...
@interface LLDynamicDetailController ()
<
...
@property (nonatomic, strong) LLStatusToolView *statusToolView;
@end

@implementation LLDynamicDetailController
...
- (void)requestDynamicDetailsSuccess:(LLDynamicModel *)dynamicModel {
    ...
    // 从小世界动态进来的详情，不需要展示
    if (!self.isWorldDynamic && dynamicModel.worldId.integerValue > 0) {
        LLDynamicLayoutModel *layout = [LLDynamicLayoutModel layoutWithStatusModel:dynamicModel];
        self.statusToolView.layout = layout;
        self.statusToolView.frame = CGRectMake(20, 10, KScreenWidth-20*2, 45);
        UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, 45)];
        [headerView addSubview:self.statusToolView];
...
// 删除评论
- (void)getDeleteCommentSuccessWithCommentId:(NSString *)commentId type:(NSInteger)type {
    [XCHUDTool hideHUD];
    ...
    !_dynamicChangeCallBack ? : _dynamicChangeCallBack(self.dynamicModel, DynamicRefreshDataTypeReload);
...
// 回复评论
- (void)requestReplyCommentWithModel:(CTReplyModel *)commentModel{
    if (!commentModel) {
        return;
    }
...
- (void)onSendText:(NSString *)text atUsers:(NSArray *)atUsers {
    ...
    if (self.commentModel) {
        ...
        [[GetCore(LittleWorldCore) requestReplyCommentWithDynamicId:self.dynamicId content:text commentId:commentID] subscribeNext:^(id  _Nullable x) {
            CTReplyModel *model = [CTReplyModel modelWithJSON:x];
            [self requestReplyCommentWithModel:model];
...
- (void)deleteCommentWithDetailCommentView:(LTDetailCommentView *)commentView {
    ...
    [[GetCore(LittleWorldCore) requestDynamicDeleteCommentWithCommentID:commentView.commentReplyModel.commentId deleteType:0] subscribeNext:^(id  _Nullable x) {
        CTReplyModel *model = [CTReplyModel modelWithJSON:x];
        [self getDeleteCommentSuccessWithCommentId:model.replyId type:0];
    } error:^(NSError * _Nullable error) {
        [XCHUDTool hideHUD];
    }];
}
...
- (void)deleteReplyWithReplyCell:(LTReplyCell *)cell {
    [XCHUDTool showGIFLoading];
    [[GetCore(LittleWorldCore) requestDynamicDeleteCommentWithCommentID:cell.replyModel.replyId deleteType:1] subscribeNext:^(id  _Nullable x) {
        CTReplyModel *model = [CTReplyModel modelWithJSON:x];
        [self getDeleteCommentSuccessWithCommentId:model.replyId type:1];
    } error:^(NSError * _Nullable error) {
        [XCHUDTool hideHUD];
    }];
...
- (void)didClickMoreButtonDynamicCell:(LTDynamicCell *)cell {
    ...
    // 删除功能
    if ([cell.dynamicModel.uid isEqualToString:currentUid] ||
        [self.worldUid isEqualToString:currentUid]) {
...
- (void)didClickAnchorOrderDynamicCell:(LTDynamicCell *)cell {
    ...
    [TTStatisticsService trackEvent:@"world_moment_chat_b" eventDescribe:@"动态详情"];
...
- (void)didClickAnchorOrderMarkDynamicCell:(LTDynamicCell *)cell {
    ...
    [TTStatisticsService trackEvent:@"world_order_explain" eventDescribe:@"动态详情"];
...
//    [self.view addSubview:self.keyBoardCloseView];
    ...
    [self addNavigationItemWithImageNames:@[@"guild_session_nav_more"] isLeft:NO target:self action:@selector(onClickRightAction:) tags:nil];
...
- (void)initSessionInput {
    self.sessionInputView = [[CTInputView alloc] initWithFrame:CGRectMake(0, KScreenHeight, KScreenWidth, 54)];
...
- (void)onClickRightAction:(id)sender {
    LTDynamicCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
    [self didClickMoreButtonDynamicCell:cell];
}
    ...
    if (index == 0) { // 点赞
        ...
        [GetCore(LittleWorldCore) requestDynamicLikeWitDynamicId:self.dynamicId
            ...
            if (success) {
                ...
                !self.dynamicChangeCallBack ? : self.dynamicChangeCallBack(self.dynamicModel, DynamicRefreshDataTypeReload);
            }
        }];
...
- (void)handShare:(SharePlatFormType)shareType {
    ...
    LLDynamicModel *dynamicModel = self.dynamicModel;
...
- (void)didSelectActionAtToolView:(LLStatusToolView *)toolView {
    ...
    vc.worldId = toolView.layout.dynamicModel.worldId;
    vc.isFromRoom = NO;
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (NSAttributedString *)commentCountAttString {
    ...
    NSMutableAttributedString *countAtt = [[NSMutableAttributedString alloc] initWithString:text attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14], NSForegroundColorAttributeName : XCThemeThirdTextColor}];
    
    NSRange range = [text rangeOfString:countStr];
   
    [countAtt yy_setColor:XCThemeMainTextColor range:range];
...
- (LLStatusToolView *)statusToolView {
    if (!_statusToolView) {
        _statusToolView = [[LLStatusToolView alloc] init];
        _statusToolView.delegate = self;
    }
    return _statusToolView;
}
@end
