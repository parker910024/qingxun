...
typedef NS_ENUM(NSInteger, SegmentViewextraBtnPosition){
    SegmentViewextraBtnPositionRight= 0,
    SegmentViewextraBtnPositionLeft = 1
};

@interface ZJSegmentStyle : NSObject

/**按钮显示左边还是右边*/
@property (nonatomic, assign) SegmentViewextraBtnPosition extraBtnPosition;
/** 是否显示遮盖 默认为NO */
...
/** 滚动条的圆角 */
@property (assign, nonatomic) CGFloat scrollLineRadius;
/** 遮盖的颜色 */
...
/** 标题是否居中   这个属性只在 scrollTitle = NO 才 生效 */
@property (assign, nonatomic) BOOL segmentInCenter;
/**  标题   Y 偏移   */
@property (assign, nonatomic) CGFloat titleOffsetY;//Y 偏移
