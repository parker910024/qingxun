...
- (void)configModel {
    if (self.message.session.sessionType == NIMSessionTypeChatroom) {
//        dispatch_async(dispatch_get_main_queue(), ^{
            [[PublicChatroomMessageHelper shareHelper] configAttrSyncWithMessage:self.message andModel:self];
//        });
    }
}
...
- (BOOL)isMe {
    return [self.message.from integerValue] == [[GetCore(AuthCore) getUid] integerValue];
}
