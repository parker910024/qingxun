...
@interface TTLittleWorldToolBar ()
<
...
@implementation TTLittleWorldToolBar
...
- (void)initConstrations {
    ...
    [self.emoticonBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.width.mas_equalTo(35);
        make.right.mas_equalTo(self.photoButton.mas_left).offset(-10);
        make.centerY.mas_equalTo(self);
    }];
    [self.inputtTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self).offset(15);
        make.right.mas_equalTo(self.emoticonBtn.mas_left).offset(-15);
...
- (void)willChangeHeight:(CGFloat)height {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(toolBarWillChangeHeight:)]) {
...
- (void)didChangeHeight:(CGFloat)height {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(toolBarDidChangeHeight:)]) {
...
- (UIButton *)photoButton {
    if (!_photoButton) {
        ...
        [_photoButton setImage:[UIImage imageNamed:@"littleworld_team_input_photo_normal"] forState:UIControlStateNormal];
        [_photoButton setImage:[UIImage imageNamed:@"littleworld_team_input_photo_select"] forState:UIControlStateHighlighted];
...
- (UIButton *)emoticonBtn {
    if (!_emoticonBtn) {
        ...
        [_emoticonBtn setImage:[UIImage imageNamed:@"littleworld_team_input_emotic"] forState:UIControlStateNormal];
        [_emoticonBtn setImage:[UIImage imageNamed:@"littleworld_team_input_emotic"] forState:UIControlStateHighlighted];
...
@implementation TTLittleWorldToolBar(InputText)

- (void)setEnable:(BOOL)isEnable {
    self.inputtTextField.editable = isEnable;
    self.inputtTextField.selectable = isEnable;
    
    self.emoticonBtn.enabled = isEnable;
    self.photoButton.enabled = isEnable;
}
...
- (void)deleteText:(NSRange)range {
    if (!self.photoButton.enabled || !self.emoticonBtn.enabled) {
        return;
    }
