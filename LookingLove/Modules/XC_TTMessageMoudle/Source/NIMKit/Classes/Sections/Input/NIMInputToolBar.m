...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [_moreMediaBtn setImage:[UIImage imageNamed:@"chat_icon_close"] forState:UIControlStateSelected];
...
- (void)update:(NIMInputStatus)status
{
    ...
    if (status == NIMInputStatusText || status == NIMInputStatusMore)
    {
        ...
        if (status == NIMInputStatusMore) {
            self.moreMediaBtn.selected = YES;
        } else {
            self.moreMediaBtn.selected = NO;
        }
    }
...
- (void)setEnable:(BOOL)isEnable {
    self.voiceButton.enabled = isEnable;
    self.inputTextView.editable = isEnable;
    self.inputTextView.selectable = isEnable;
    self.emoticonBtn.enabled = isEnable;
    self.moreMediaBtn.enabled = isEnable;
}
