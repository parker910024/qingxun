...
@property (nonatomic, assign) BOOL isShowPrivateMessage;
@end
...
- (NSArray<NSIndexPath *> *)deleteModels:(NSRange)range
{
    ...
    for (NIMMessageModel *model in models) {
        ...
        if (delMsgIndex > -1) {
            [self.items removeObjectAtIndex:delMsgIndex];
...
- (NSArray<NIMMessageModel *> *)modelsWithMessages:(NSArray<NIMMessage *> *)messages
{
    ...
    for (NIMMessage *message in messages) {
        NIMMessageModel *model = [[NIMMessageModel alloc] initWithMessage:message];
        if ([self handleRedTipsMsg:message]) {
...
- (BOOL)handleRedTipsMsg:(NIMMessage *)message {
...
//                            message.localExt = att.data;
                        ...
                        if (att.second == Custom_Noti_Sub_CPGAME_PrivateChat_LaunchGame) {
                            TTGameCPPrivateChatModel *messageModel = [TTGameCPPrivateChatModel modelDictionary:att.data];
                            for (int i = 0 ; i < GetCore(TTGameStaticTypeCore).privateMessageArray.count; i++) {
                                ...
                                if ([gameModel.gameId isEqualToString:messageModel.gameInfo.gameId]) {
                                    self.isShowPrivateMessage = YES;
                                    break;
                                } else {
                                    self.isShowPrivateMessage = NO;
                                }
                            }
                            if (self.isShowPrivateMessage) {
                                return YES;
                            } else {
                                return NO;
                            }
