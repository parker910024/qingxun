...
@interface TTRoomDisableViewController ()
...
@implementation TTRoomDisableViewController
...
//    CGFloat transformY = keyboardFrame.origin.y - self.view.frame.size.height;
    ...
    //执行动画
    [self.disableSendView layoutIfNeeded];
    
    [UIView animateWithDuration:duration animations:^{
        [self.disableSendView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.centerY.mas_equalTo(self.view.mas_centerY).offset(-kScale(130));
        }];
        self.disableSendView.centerY = KScreenHeight / 2 - kScale(130);
    }];
}
...
- (void)keyboardWillHidden:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:duration animations:^{
        self.disableSendView.centerY = KScreenHeight / 2;
        [self.disableSendView mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)initView {
    ...
    [self.view addSubview:self.disableSendView];
...
- (void)initConstraint {
    ...
    @weakify(self);
    [[GetCore(UserCore) getUserInfoByRac:self.userID refresh:NO] subscribeNext:^(id x) {
        @strongify(self);
...
- (void)sureBtnAction:(UIButton *)sender {
    ...
    [GetCore(PublicChatroomCore) publicChatRoomNotMessage:self.userID duration:[[self.timeSecondArray safeObjectAtIndex:index] intValue] remark:remark];
...
- (XCPlaceholderTextView *)reasonTextView {
    if (!_reasonTextView) {
        ...
        _reasonTextView.font = [UIFont systemFontOfSize:12.0];
        ...
        [_reasonTextView setPlaceholderFont:[UIFont systemFontOfSize:12.0]];
