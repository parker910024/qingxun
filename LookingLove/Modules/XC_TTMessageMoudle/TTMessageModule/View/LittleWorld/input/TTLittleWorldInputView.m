...
@interface TTLittleWorldInputView ()

<
TTLittleWorldToolBarDelegate,
NIMInputEmoticonProtocol,
NIMContactSelectDelegate,
UserCoreClient
>
@property (nonatomic, weak) id<TTLittleWorldInputViewDelegate> inputDelegate;
@property (nonatomic, weak) id<NIMInputActionDelegate> actionDelegate;

/** 用户的个人信息*/
@property (nonatomic,strong) UserInfo *myInfor;;

@end

@implementation TTLittleWorldInputView

@synthesize emojiContainer = _emojiContainer;

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        AddCoreClient(UserCoreClient, self);
        [self initView];
        self.myInfor = [GetCore(UserCore)getUserInfoInDB:[GetCore(AuthCore)getUid].userIDValue];
    }
    return self;
}
...
- (void)initView {
    ...
    if (!_toolBar) {
        _toolBar = [[TTLittleWorldToolBar alloc]initWithFrame:CGRectMake(0, 0, self.nim_width, 0)];
        [self addSubview:_toolBar];
        _toolBar.delegate = self;
        if (!GetCore(LittleWorldCore).worldGroupChatLevelNo || self.myInfor.userLevelVo.experLevelSeq.integerValue < GetCore(LittleWorldCore).worldGroupChatLevelNo) {
            [_toolBar setEnable:NO];
        }else {
            [_toolBar setEnable:YES];
        }
        
        if (self.myInfor.userLevelVo.experLevelSeq.integerValue < GetCore(LittleWorldCore).worldGroupChatLevelNo) {
            [_toolBar setText:[NSString stringWithFormat:@"用户等级不足%ld级，不可以发言哦~", GetCore(LittleWorldCore).worldGroupChatLevelNo]];
        }
        //设置placeholder
        [_toolBar setPlaceHolder:[NSString stringWithFormat:@"请输入消息"]];
        [_toolBar.emoticonBtn addTarget:self action:@selector(onTouchEmojiButton:) forControlEvents:UIControlEventTouchUpInside];
        [_toolBar.photoButton addTarget:self action:@selector(onTouchPhotoButton:) forControlEvents:UIControlEventTouchUpInside];
        self.maxTextLength = 40;
        [self refreshStatus:TTLittleWorldInputStatusText];
        [self sizeToFit];
    }
...
#pragma mark - TTLittleWorldInputToolBarDelegate
...
- (void)onTouchEmojiButton:(UIButton *)sender {
    if (self.status != TTInputStatusEmoticon) {
        ...
        [self bringSubviewToFront:_emojiContainer];
        [_emojiContainer setHidden:NO];
        [self refreshStatus:TTLittleWorldInputStatusEmoticon];
        [self sizeToFit];
...
- (void)onCurrentUserInfoUpdate:(UserInfo *)userInfo {
    if (userInfo.uid == [GetCore(AuthCore)getUid].userIDValue) {
        self.myInfor = userInfo;
    }
}

#pragma mark - 点击图片的
- (void)onTouchPhotoButton:(UIButton *)sender {
    if (self.inputDelegate && [self.inputDelegate respondsToSelector:@selector(didClickInputViewPhoto)]) {
        [self.inputDelegate didClickInputViewPhoto];
    }
}


#pragma mark - setter & getter
- (void)setMyInfor:(UserInfo *)myInfor {
    _myInfor = myInfor;
    if (_myInfor.userLevelVo.experLevelSeq.integerValue < GetCore(LittleWorldCore).worldGroupChatLevelNo) {
        [self.toolBar setEnable:NO];
        [self.toolBar setText:[NSString stringWithFormat:@"用户等级不足%ld级，不可以发言哦~", GetCore(LittleWorldCore).worldGroupChatLevelNo]];
    }else {
