...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error{
    NSLog(@"request ipa didFailWithError:%@",error);
}
...
- (void)buyProduct:(SKProduct *)productIdentifier orderId:(NSString *)orderID onCompletion:(IAPbuyProductCompleteResponseBlock)completion {
    ...
    //modify by KevinWang - 2018-3-7
    SKMutablePayment *payment = [SKMutablePayment paymentWithProduct:productIdentifier];
    //    payment.requestData = [[NSData alloc] initWithBase64EncodedString:<#(nonnull NSString *)#> options:<#(NSDataBase64DecodingOptions)#>];
