...
+ (NIMMessage*)msgWithText:(NSString*)text
{
    ...
    textMessage.text        = [GetCore(ImMessageCore) antiSpam:text withMessage:textMessage];
    [GetCore(MessageCollection)sendMessageCollectionRequestWithMessage:textMessage firstAttachment:0];
    return textMessage;
}
...
+ (NIMMessage*)msgWithAudio:(NSString*)filePath
{
    ...
    [GetCore(MessageCollection)sendMessageCollectionRequestWithMessage:message firstAttachment:2];
    return message;
}
...
+ (NIMMessage*)msgWithVideo:(NSString*)filePath
{
    ...
    [GetCore(MessageCollection)sendMessageCollectionRequestWithMessage:message firstAttachment:3];
    return message;
}
...
+ (NIMMessage *)generateImageMessage:(NIMImageObject *)imageObject
{
    ...
    [GetCore(MessageCollection)sendMessageCollectionRequestWithMessage:message firstAttachment:1];
    return message;
}


+ (NIMMessage*)msgWithLocation:(NIMKitLocationPoint *)locationPoint{
    NIMLocationObject *locationObject = [[NIMLocationObject alloc] initWithLatitude:locationPoint.coordinate.latitude longitude:locationPoint.coordinate.longitude title:locationPoint.title];
    NIMMessage *message               = [[NIMMessage alloc] init];
    message.messageObject             = locationObject;
    message.apnsContent = @"发来了一条位置信息";
    [GetCore(MessageCollection)sendMessageCollectionRequestWithMessage:message firstAttachment:4];
    return message;
}
                          ...
                          toRobot:(NSString *)robotId
{
    ...
    [GetCore(MessageCollection)sendMessageCollectionRequestWithMessage:queryMessage firstAttachment:11];
    return queryMessage;
}
