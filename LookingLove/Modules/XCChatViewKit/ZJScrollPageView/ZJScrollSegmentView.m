...
//标题不可滑动 并 居中 的 容器View
@property (nonatomic, strong) UIView  *contenetView;//
...
- (void)setupScrollViewAndExtraBtn {
    ...
    if (self.extraBtn) {
        if (self.segmentStyle.extraBtnPosition ==SegmentViewextraBtnPositionLeft) {
            self.extraBtn.frame = CGRectMake(0 , extraBtnY, extraBtnW, self.zj_height - 2*extraBtnY);
            self.scrollView.frame = CGRectMake(extraBtnW - 15, 0.0, scrollW + 15, self.zj_height);
        }else{
...
- (void)setUpTitleViewsPosition {
    ...
    if (!self.segmentStyle.isScrollTitle) {
        ...
        NSInteger index = 0;
        NSInteger subViewCount = self.titleViews.count;
        
        
        if (self.segmentStyle.segmentInCenter) {
            titleW = [self.titleWidths[index] floatValue];
            self.contenetView.frame = CGRectMake(0, 0, subViewCount*titleW+(subViewCount+1)*self.segmentStyle.titleMargin, titleH);
            
            for (ZJTitleView *titleView in self.titleViews) {
                titleX = index * titleW;
                titleView.frame = CGRectMake(titleX+self.segmentStyle.titleMargin*(index+1), titleY, titleW, titleH);
                
                if (self.segmentStyle.isShowImage) {
                    [titleView adjustSubviewFrame];
                }
                [self.contenetView addSubview:titleView];
                index++;
            }
            
            [self.scrollView addSubview:self.contenetView];
            self.contenetView.center = CGPointMake(self.scrollView.bounds.size.width/2, self.contenetView.center.y);
...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            if(self.segmentStyle.segmentInCenter){
                [self.contenetView addSubview:self.scrollLine];
                
                coverX = firstLabel.zj_x;
                coverW = firstLabel.zj_width;
                
                
            }
            
            self.scrollLine.frame = CGRectMake(coverX , self.zj_height - self.segmentStyle.scrollLineHeight, 23 , self.segmentStyle.scrollLineHeight);
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    ...
                    if (self.segmentStyle.segmentInCenter) {
                        scrollLineW = [self.titleWidths[_currentIndex] floatValue];
                        scrollLineX = currentTitleView.zj_x;
                    }
                    ...
                    weakSelf.scrollLine.zj_width = 23;
                }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    CGFloat xDistance = currentTitleView.zj_centerX - oldTitleView.zj_centerX;
...
- (void)reloadTitlesWithNewTitles:(NSArray *)titles withIndex:(NSInteger)index{
    [self.scrollView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    _currentIndex = index;
    _oldIndex = index;
    self.titleWidths = nil;
...
- (UIView *)scrollLine {
    ...
    if (!_scrollLine) {
        ...
        if (self.segmentStyle.scrollLineRadius>0) {
            lineView.layer.masksToBounds = YES;
            lineView.layer.cornerRadius = self.segmentStyle.scrollLineRadius;
        }
...
- (UIView *)contenetView {
    if (!_contenetView){
        _contenetView = [[UIView alloc] init];
    }
    return _contenetView;
}
