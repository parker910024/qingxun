...
const char xc_unz_copyright[] = " unzip 1.2.0 Copyright 1998-2017 - https://github.com/nmoinvaz/minizip";
...
static unzFile unzOpenInternal(const void *path, zlib_filefunc64_32_def *pzlib_filefunc64_32_def)
{
    ...
    if (pzlib_filefunc64_32_def == NULL)
        xc_fill_fopen64_filefunc(&us.z_filefunc.zfile_func64);
    else
    {
    ...
    if (s != NULL)
    {
        *s = us;
        ...
        xc_unzGoToFirstFile((unzFile)s);
    }
    return (unzFile)s;
}

extern unzFile ZEXPORT xc_unzOpen2(const char *path, zlib_filefunc_def *pzlib_filefunc32_def)
{
    if (pzlib_filefunc32_def != NULL)
    {
        zlib_filefunc64_32_def zlib_filefunc64_32_def_fill = { 0 };
        xc_fill_zlib_filefunc64_32_def_from_filefunc32(&zlib_filefunc64_32_def_fill, pzlib_filefunc32_def);
        return unzOpenInternal(path, &zlib_filefunc64_32_def_fill);
    }
    return unzOpenInternal(path, NULL);
}

extern unzFile ZEXPORT xc_unzOpen2_64(const void *path, zlib_filefunc64_def *pzlib_filefunc_def)
{
...
extern unzFile ZEXPORT xc_unzOpen(const char *path)
{
    return unzOpenInternal(path, NULL);
}

extern unzFile ZEXPORT xc_unzOpen64(const void *path)
{
    return unzOpenInternal(path, NULL);
}

extern int ZEXPORT xc_unzClose(unzFile file)
{
    unz64_internal *s;
    if (file == NULL)
...
extern int ZEXPORT xc_unzGetGlobalInfo(unzFile file, unz_global_info* pglobal_info32)
{
    unz64_internal *s = NULL;
    if (file == NULL)
        return UNZ_PARAMERROR;
...
extern int ZEXPORT xc_unzGetGlobalInfo64(unzFile file, unz_global_info64 *pglobal_info)
{
    unz64_internal *s = NULL;
    if (file == NULL)
        return UNZ_PARAMERROR;
...
extern int ZEXPORT xc_unzGetGlobalComment(unzFile file, char *comment, uint16_t comment_size)
{
    unz64_internal *s = NULL;
...
extern int ZEXPORT xc_unzGetCurrentFileInfo(unzFile file, unz_file_info *pfile_info, char *filename,
...
extern int ZEXPORT xc_unzGetCurrentFileInfo64(unzFile file, unz_file_info64 * pfile_info, char *filename,
    uint16_t filename_size, void *extrafield, uint16_t extrafield_size, char *comment, uint16_t comment_size)
{
...
*/
extern int ZEXPORT xc_unzOpenCurrentFile3(unzFile file, int *method, int *level, int raw, const char *password)
{
    unz64_internal *s = NULL;
...
extern int ZEXPORT xc_unzOpenCurrentFile(unzFile file)
{
    return xc_unzOpenCurrentFile3(file, NULL, NULL, 0, NULL);
}

extern int ZEXPORT xc_unzOpenCurrentFilePassword(unzFile file, const char *password)
{
    return xc_unzOpenCurrentFile3(file, NULL, NULL, 0, password);
}

extern int ZEXPORT xc_unzOpenCurrentFile2(unzFile file, int *method, int *level, int raw)
{
    return xc_unzOpenCurrentFile3(file, method, level, raw, NULL);
}
...
extern int ZEXPORT xc_unzReadCurrentFile(unzFile file, voidp buf, uint32_t len)
{
    unz64_internal *s = NULL;
    uint32_t read = 0;
    int err = UNZ_OK;
...
extern int ZEXPORT xc_unzGetLocalExtrafield(unzFile file, voidp buf, uint32_t len)
{
    unz64_internal *s = NULL;
...
extern int ZEXPORT xc_unzCloseCurrentFile(unzFile file)
{
    unz64_internal *s = NULL;
    file_in_zip64_read_info_s *pfile_in_zip_read_info = NULL;
    int err = UNZ_OK;
...
extern int ZEXPORT xc_unzGoToFirstFile(unzFile file)
{
...
extern int ZEXPORT xc_unzGoToNextFile(unzFile file)
{
    return unzGoToNextFile2(file, NULL, NULL, 0, NULL, 0, NULL, 0);
}

extern int ZEXPORT xc_unzLocateFile(unzFile file, const char *filename, unzFileNameComparer filename_compare_func)
{
    unz64_internal *s = NULL;
...
extern int ZEXPORT xc_unzGetFilePos(unzFile file, unz_file_pos *file_pos)
{
    unz64_file_pos file_pos64;
    int err = xc_unzGetFilePos64(file, &file_pos64);
    if (err == UNZ_OK)
    {
...
extern int ZEXPORT xc_unzGoToFilePos(unzFile file, unz_file_pos *file_pos)
{
    unz64_file_pos file_pos64;
    if (file_pos == NULL)
        return UNZ_PARAMERROR;
    ...
    return xc_unzGoToFilePos64(file, &file_pos64);
}

extern int ZEXPORT xc_unzGetFilePos64(unzFile file, unz64_file_pos *file_pos)
{
    unz64_internal *s = NULL;
...
extern int ZEXPORT xc_unzGoToFilePos64(unzFile file, const unz64_file_pos *file_pos)
{
    unz64_internal *s = NULL;
    int err = UNZ_OK;
...
extern int32_t ZEXPORT xc_unzGetOffset(unzFile file)
{
    uint64_t offset64 = 0;

    if (file == NULL)
        return UNZ_PARAMERROR;
    offset64 = xc_unzGetOffset64(file);
    return (int32_t)offset64;
}

extern int64_t ZEXPORT xc_unzGetOffset64(unzFile file)
{
    unz64_internal *s = NULL;
...
extern int ZEXPORT xc_unzSetOffset(unzFile file, uint32_t pos)
{
    return xc_unzSetOffset64(file, pos);
}

extern int ZEXPORT xc_unzSetOffset64(unzFile file, uint64_t pos)
{
    unz64_internal *s = NULL;
    int err = UNZ_OK;
