...
+ (NSMutableAttributedString *)messageTipContent:(NIMMessage *)message{
    ...
    if (text == nil) {
        switch (message.messageType) {
            ...
            case NIMMessageTypeTip:
                text = [[NSMutableAttributedString alloc]initWithString:message.text];
                break;
            default:
                break;
        }
...
+ (NSString *)messageNormalTipContent:(NIMMessage *)message {
    NSString *text = nil;
    if (text == nil) {
        switch (message.messageType) {
            case NIMMessageTypeNotification:
                text =  [NIMKitUtil notificationMessageWithNormalStr:message];
                break;
            case NIMMessageTypeTip:
                text = message.text;
                break;
...
+ (NSString *)notificationMessageWithNormalStr:(NIMMessage *)message {
...
+ (NSMutableAttributedString *)notificationMessage:(NIMMessage *)message{
    NIMNotificationObject *object = message.messageObject;
    switch (object.notificationType) {
        case NIMNotificationTypeTeam:{
            NSString *content = [NIMKitUtil teamNotificationFormatedMessage:message];
            NSRange annouceRange = [content rangeOfString:@"群公告："];
            NSMutableAttributedString *attr = [[NSMutableAttributedString alloc]initWithString:content];
            [attr addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:1 green:83/255.0 blue:83/255.0 alpha:1] range:annouceRange];
            
            return attr;
        }
        case NIMNotificationTypeNetCall:{
            return [[NSMutableAttributedString alloc]initWithString:[NIMKitUtil netcallNotificationFormatedMessage:message]];
        }
        case NIMNotificationTypeChatroom:{
            return [[NSMutableAttributedString alloc]initWithString:[NIMKitUtil chatroomNotificationFormatedMessage:message]];
        }
        default:
                    ...
                    if ([teamAttachment.values count] == 1) {
                        ...
                        switch (tag) {
                            ...
                            case NIMTeamUpdateTagAnouncement:
                                ...
                                formatedMessage = [NSString stringWithFormat:@"%@公告：%@", teamName, teamAttachment.values.allValues.firstObject];
                                break;
