...
@interface XCPlaceholderTextView()<UITextViewDelegate>
...
@implementation XCPlaceholderTextView
...
-(void)textViewBeginNoti:(NSNotification*)noti{
    ...
    if (_BeginBlock) {
        void(^begin)(XCPlaceholderTextView*text)=_BeginBlock;
        begin(self);
    }
}
-(void)textViewEndNoti:(NSNotification*)noti{
    ...
    if (_EndBlock) {
        void(^end)(XCPlaceholderTextView*text)=_EndBlock;
        end(self);
    }
}
...
-(void)DidChange:(NSNotification*)noti{
    ...
    if (_updateBlock) {
        void (^update)(XCPlaceholderTextView*text) =_updateBlock;
        update(self);
    }
    
    if (_eventBlock && self.text.length > self.maxTextLength) {
        
        void (^limint)(XCPlaceholderTextView*text) =_eventBlock;
...
-(void)addTextViewBeginEvent:(void (^)(XCPlaceholderTextView *))begin{
...
-(void)addTextViewEndEvent:(void (^)(XCPlaceholderTextView *))End{
    _EndBlock=End;
}
...
+ (float)boundingRectWithSize:(CGSize)size withLabel:(NSString *)label withFont:(UIFont *)font{
    ...
    // CGSize retSize;
    CGSize retSize = [label boundingRectWithSize:size  options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
