...
@interface XCSSZipArchive ()
- (instancetype)init NS_DESIGNATED_INITIALIZER;
@end

@implementation XCSSZipArchive
{
    /// path for zip file
    NSString *_path;
    zipFile _zip;
...
+ (BOOL)isFilePasswordProtectedAtPath:(NSString *)path {
    // Begin opening
    zipFile zip = xc_unzOpen(path.fileSystemRepresentation);
    if (zip == NULL) {
        return NO;
    }
    
    int ret = xc_unzGoToFirstFile(zip);
    if (ret == UNZ_OK) {
        do {
            ret = xc_unzOpenCurrentFile(zip);
            if (ret != UNZ_OK) {
                // attempting with an arbitrary password to workaround `unzOpenCurrentFile` limitation on AES encrypted files
                ret = xc_unzOpenCurrentFilePassword(zip, "");
                xc_unzCloseCurrentFile(zip);
            ...
            ret = xc_unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            xc_unzCloseCurrentFile(zip);
            if (ret != UNZ_OK) {
                return NO;
    ...
    if (ret == UNZ_OK) {
        do {
            if (pw.length == 0) {
                ...
                ret = xc_unzOpenCurrentFilePassword(zip, [pw cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            if (ret != UNZ_OK) {
            ...
            if (ret != UNZ_OK) {
                ...
                int readBytes = xc_unzReadCurrentFile(zip, buffer, (unsigned)MIN(10UL,fileInfo.uncompressed_size));
                if (readBytes < 0) {
      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    xc_unzGetGlobalInfo(zip, &globalInfo);
    
    // Begin unzipping
    int ret = 0;
    ret = xc_unzGoToFirstFile(zip);
    ...
    do {
        ...
        @autoreleasepool {
            if (password.length == 0) {
                ...
                ret = xc_unzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            ...
            xc_unzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            filename[fileInfo.size_filename] = '\0';
            
            BOOL fileIsSymbolicLink = _fileIsSymbolicLink(&fileInfo);
            
            NSString * strPath = [XCSSZipArchive _filenameStringWithCString:filename
            ...
            if (isDirectory && !fileIsSymbolicLink) {
                ...
                int readBytes = xc_unzReadCurrentFile(zip, buffer, 4096);
                if (readBytes >= 0) {
                    ...
                    while (fp) {
                        ...
                        readBytes = xc_unzReadCurrentFile(zip, buffer, 4096);
                        if (readBytes < 0) {
                            // Let's assume error Z_DATA_ERROR is caused by an invalid password
                            // Let's assume other errors are caused by Content Not Readable
                            success = NO;
            ...
            else
            {
                ...
                int bytesRead = 0;
                while ((bytesRead = xc_unzReadCurrentFile(zip, buffer, 4096)) > 0)
                {
                    buffer[bytesRead] = 0;
            ...
            crc_ret = xc_unzCloseCurrentFile(zip);
...
{
    return [XCSSZipArchive createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil];
}
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath {
    return [XCSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory {
    return [XCSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray<NSString *> *)paths withPassword:(NSString *)password
{
    XCSSZipArchive *zipArchive = [[XCSSZipArchive alloc] initWithPath:path];
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath withPassword:(nullable NSString *)password {
    return [XCSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password];
}


+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password {
    return [XCSSZipArchive createZipFileAtPath:path
...
- (BOOL)open
{
    ...
    _zip = xc_zipOpen(_path.fileSystemRepresentation, APPEND_STATUS_CREATE);
    return (NULL != _zip);
}
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName withPassword:(nullable NSString *)password
{
    ...
    [XCSSZipArchive zipInfo:&zipInfo setAttributesOfItemAtPath:path];
    
    int error = _zipOpenEntry(_zip, [folderName stringByAppendingString:@"/"], &zipInfo, Z_NO_COMPRESSION, password, 0);
    const void *buffer = NULL;
    xc_zipWriteInFileInZip(_zip, buffer, 0);
    xc_zipCloseFileInZip(_zip);
    return error == ZIP_OK;
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(nullable NSString *)fileName compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    while (!feof(input) && !ferror(input))
    {
        ...
        xc_zipWriteInFileInZip(_zip, buffer, len);
    }
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    zip_fileinfo zipInfo = {};
    [XCSSZipArchive zipInfo:&zipInfo setDate:[NSDate date]];
    
    int error = _zipOpenEntry(_zip, filename, &zipInfo, compressionLevel, password, aes);
    
    xc_zipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
- (BOOL)close
{
    ...
    int error = xc_zipClose(_zip, NULL);
    _zip = nil;
    return error == ZIP_OK;
}
...
{
    NSCalendar *currentCalendar = XCSSZipArchive._gregorian;
