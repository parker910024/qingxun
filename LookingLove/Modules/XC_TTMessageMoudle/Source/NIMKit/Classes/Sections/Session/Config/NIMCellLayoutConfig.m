...
{
    if ([[self cellContent:model] isEqualToString:@"NIMSessionNotificationContentView"] || [[self cellContent:model] isEqualToString:@"XCTipsContentView"]) {
...
- (BOOL)disableRetryButton:(NIMMessageModel *)model
{
    ...
    {
        if (model.message.attachmentDownloadState == NIMMessageAttachmentDownloadStateFailed) {
            if (model.message.messageObject && model.message.messageType == NIMMessageTypeImage) {
