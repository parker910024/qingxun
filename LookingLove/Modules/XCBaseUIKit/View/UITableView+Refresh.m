...
static void * KtableViewHeightOnScreen = (void *)@"KtableViewHeightOnScreens";
...
- (void)setTableViewHeightOnScreen:(CGFloat)tableViewHeightOnScreen
{
    objc_setAssociatedObject(self, &KtableViewHeightOnScreen, @(tableViewHeightOnScreen), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CGFloat)tableViewHeightOnScreen
{
    NSNumber *number =  (NSNumber *)objc_getAssociatedObject(self, &KtableViewHeightOnScreen);
...
- (void)setupRefreshFunctionWith:(RefreshType)type
{
    ...
    else if (type==RefreshTypeFooter)
    {
        ...
        self.tableViewHeightOnScreens = self.tableViewHeightOnScreen;
...
- (void)setupRefreshFunctionWith:(RefreshType)type andColor:(UIColor *)color {
    ...
    if (type == RefreshTypeHeader)
    {
        ...
        self.mj_header = [RefreshFactory headerRefreshWithTarget:self refreshingAction:@selector(headerRefresh) color:color];
    }
    else if (type==RefreshTypeFooter)
    {
        //集成底部刷新
        self.mj_footer = [RefreshFactory footerRefreshWithTarget:self refreshingAction:@selector(footerRefresh) color:color];
        self.mj_footer.automaticallyHidden = YES;//没有数据自动隐藏
        self.screenOrigins = self.screenOrigin;
        self.tableViewHeightOnScreens = self.tableViewHeightOnScreen;
        self.footRefreshState =MJFooterRefreshStateNormal;
