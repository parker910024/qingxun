...
@implementation WBRollView

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        self.clipsToBounds = YES;
        self.repectAnimation = YES;
...
- (void)dealloc {
    ...
    [self.marqueeLabel removeObserver:self forKeyPath:@"attributedText"];
...
- (void)startAnimation {
    ...
    if (size.width > CGRectGetWidth(self.bounds)) {
        ...
        if (self.repectAnimation) {
            ...
            nextLabel.font = [UIFont boldSystemFontOfSize:20];
        ...
        animation.duration = self.repectAnimation ? size.width/50.f : (rect.size.width-self.innerContainer.frame.size.width)/30;
        animation.values = @[@0, self.repectAnimation ? @(-(size.width + kLabelOffset)) : @(-(rect.size.width-self.innerContainer.frame.size.width))];
        animation.repeatCount = self.repectAnimation ? INT16_MAX : 1;
        animation.timingFunction = [CAMediaTimingFunction functionWithName:@"linear"];
        animation.removedOnCompletion = self.repectAnimation;
        animation.fillMode = self.repectAnimation ? kCAFillModeRemoved : kCAFillModeForwards;
...
- (void)stopAnimation:(CGFloat)duration {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(duration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        @KStrongify(self);
        [self.innerContainer.layer removeAllAnimations];
    });
