...
// 点赞的cell
@property (nonatomic, weak) LTDynamicCell *likeCell;
///当前正在播放的index
...
@implementation LTDynamicCircleVC
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.dataArray.count > indexPath.row) {
        cell.dynamicModel = self.dataArray[indexPath.row];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self jumpToDynamicDeatailVcWithDynamicModel:dynamicModel indexPath:indexPath replyComment:nil isShowKeyboard:NO];
}
...
- (void)didClickCommentButtonWithMyVestDynamicCell:(LTDynamicCell *)cell {
    ...
    [self jumpToDynamicDeatailVcWithDynamicModel:dynamicModel indexPath:indexPatch replyComment:nil isShowKeyboard:YES];
    self.deleteCell = cell;
}
...
- (void)requestDynamicListSuccess:(NSArray<CTDynamicModel *> *)modelList nextDynamicId:(nonnull NSString *)nextDynamicId{
    ...
    if (self.dataArray.count == 0) {
        [self.tableView showEmptyContentToastWithTitle:@"这个小世界的历史由你撰写~" andImage:[UIImage imageNamed:@"dynamic_data_empty_icon"]];
    } else {
        [self.tableView hideToastView];
    }

    self.nextDynamicID = nextDynamicId;
...
- (void)requestDynamicGetSquareSuccess:(NSArray<CTDynamicModel *> *)modelList {
    if (self.type != LTDynamicCircleTypePlayground) return;
...
///跳转到详情
- (void)jumpToDynamicDeatailVcWithDynamicModel:(CTDynamicModel *)dynamicModel indexPath:(NSIndexPath *)indexPath replyComment:(CTCommentReplyModel *)replyComment isShowKeyboard:(BOOL)isShowKeyboard {
    ...
    LTDynamicDetailVC *detailsVc = [[LTDynamicDetailVC alloc]init];
    detailsVc.dynamicId = dynamicModel.dynamicId;
    detailsVc.worldId = self.worldID;
    detailsVc.worldItem = self.worldItem;
    detailsVc.isShowKeyboard = isShowKeyboard;
    detailsVc.commentModel = replyComment;
    if (replyComment) {//点击评论
        detailsVc.changeSection = [dynamicModel.commentVoList indexOfObject:replyComment] + 1;
    }
    [detailsVc setDynamicChangeCallBack:^(CTDynamicModel * _Nonnull newDynamic) {
        dynamicModel.isLike = newDynamic.isLike;
        dynamicModel.likeCount = newDynamic.likeCount;
        dynamicModel.commentCount = newDynamic.commentCount;
        [UIView performWithoutAnimation:^{
