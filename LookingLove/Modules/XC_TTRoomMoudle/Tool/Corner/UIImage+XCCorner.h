...
/**
 ...
 将要被渲染到图片的layer
 @param radius The radiuses of 4 corners. If you pass XCRadiusZero, the final image will not add corner
 4个圆角的半径，如果传入XCRadiusZero，最终的图片将不添加圆角
 */
...
/**
 ...
 4个圆角的半径，如果传入XCRadiusZero，最终的图片将不添加圆角
 */
+ (UIImage *)imageWithGradualChangingColor:(void(^)(XCGradualChangingColor *graColor))handler size:(CGSize)size cornerRadius:(XCRadius)radius;

/**
 Create a UIImage with border and corner. Always uses in UIButton
 创建一个边框图片，可以带圆角。通常在UIButton使用
 @param corner The properities of corner, see XCCorner.
 corner的属性，看XCCorner的介绍
 @param size The size of the image.
 图片的尺寸
 */
+ (UIImage *)imageWithXCCorner:(void(^)(XCCorner *corner))handler size:(CGSize)size;
