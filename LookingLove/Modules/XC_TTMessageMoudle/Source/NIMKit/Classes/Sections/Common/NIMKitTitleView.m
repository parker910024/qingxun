...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _subtitleLabel = [[YYLabel alloc] initWithFrame:CGRectZero];
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    YYTextLayout *layout = [YYTextLayout layoutWithContainerSize:CGSizeMake(maxWidth, maxWidth) text:self.subtitleLabel.attributedText];
    CGFloat subtitleWidth = layout.textBoundingSize.width;//self.subtitleLabel.nim_width算不准
    
    [self.subtitleLabel sizeToFit];
    self.subtitleLabel.nim_width = MIN(subtitleWidth, maxWidth);
