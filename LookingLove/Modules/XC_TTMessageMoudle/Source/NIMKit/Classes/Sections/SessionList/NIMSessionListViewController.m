...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        [self onSelectedServiceAtIndexPath:indexPath];
    }else{
        if (self.recentSessions.count > 0) {
            NIMRecentSession *recentSession = [self.recentSessions safeObjectAtIndex:indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        TTQYServiceTableViewCell * qyserviceCell = [tableView dequeueReusableCellWithIdentifier:@"TTQYServiceTableViewCell"];
        if (qyserviceCell == nil) {
            qyserviceCell = [[TTQYServiceTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"TTQYServiceTableViewCell"];
        }
//        [qyserviceCell refresh];
        return qyserviceCell;
    }
    ...
    //modify by KevinWang 2018-3-9
    NIMRecentSession *recent = [self.recentSessions safeObjectAtIndex:indexPath.row];
