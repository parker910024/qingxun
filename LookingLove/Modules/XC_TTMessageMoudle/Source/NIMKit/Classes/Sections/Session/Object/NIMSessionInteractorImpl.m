...
- (void)addMessages:(NSArray *)messages
{
    ...
    if (message.session.sessionType == NIMSessionTypeChatroom) {
        ...
        if ([self.delegate isKindOfClass:NSClassFromString(@"TTLittleWorldSessionViewController")] && message.session.sessionType == NIMSessionTypeTeam && message.messageType == NIMMessageTypeNotification) {
            NIMNotificationObject * object = message.messageObject;
            NIMTeamNotificationContent *content = (NIMTeamNotificationContent*)object.content;
            if (content.operationType != NIMTeamOperationTypeLeave) {
...
- (void)addNormalMessages:(NSArray *)messages
{
    ...
    for (NIMMessage *message in messages) {
        ...
        if ([self handleTheMessageWithLoacExt:message]) {
            NIMMessageModel *model = [[NIMMessageModel alloc] initWithMessage:message];
            [models addObject:model];
        }
    }
...
- (BOOL)handleTheMessageWithLoacExt:(NIMMessage *)message {
    if (message.messageObject && message.messageType == NIMMessageTypeCustom) {
            ...
            if (customObject.attachment != nil && [customObject.attachment isKindOfClass:[Attachment class]]) {
                ...
                if (att.first == Custom_Noti_Header_Group_RedPacket) {
                     if (att.second == Custom_Noti_Sub_Header_Group_RedPacket_Send) {
                        message.localExt = att.data;
                    }else if (att.second == Custom_Noti_Sub_Header_Group_RedPacket_Tips) {
                        RedPacketDetailInfo *info = [RedPacketDetailInfo yy_modelWithDictionary:att.data];
                        if (info.receiveUid == [GetCore(AuthCore)getUid].userIDValue || info.senderUid == [GetCore(AuthCore)getUid].userIDValue) {
                            return YES;
                        }else {
                ...
                }else if (att.first == Custom_Noti_Header_Message_Handle) {
                    if (att.second == Custom_Noti_Sub_Header_Message_Handle_Bussiness) {
                        message.localExt = att.data;
                    }
                } else if (att.first == Custom_Noti_Header_OfficialAnchorCertification) {//复制了上一个if
                    if (att.second == Custom_Noti_Sub_OfficialAnchorCertification_Bussiness) {
                        message.localExt = att.data;
                    }
                }
            }
...
- (void)addChatroomMessages:(NSArray *)messages
{
    ...
    dispatch_async(NTESMessageDataPrepareQueue(), ^{
        ...
        for (NIMMessage *message in messages)
        {
            ...
            if (!(message.session.sessionType == NIMSessionTypeChatroom && [message.session.sessionId integerValue] == [self.session.sessionId integerValue] && (message.messageType == NIMMessageTypeCustom || message.messageType == NIMMessageTypeText))) {
                continue;
            }
            NIMCustomObject *customObject = (NIMCustomObject *)message.messageObject;
            if ([customObject.attachment isKindOfClass:Attachment.class]) {
...
- (void)tableViewIsInBottom:(BOOL)isInBottom {
    if ([self.layout respondsToSelector:@selector(tableViewCurrentIsInBottom:)]) {
        [self.layout tableViewCurrentIsInBottom:isInBottom];
    }
}
...
- (void)loadMessages:(void (^)(NSArray *messages, NSError *error))handler
{
    ...
    [self.dataSource loadHistoryMessagesWithComplete:^(NSInteger index, NSArray *messages, NSError *error) {
        ...
        //过滤掉历史数据小世界群聊离开群聊的通知消息
        NSMutableArray *updateMsgs = [NSMutableArray arrayWithArray:messages];
        BOOL littleWorldSession = [self.delegate isKindOfClass:NSClassFromString(@"TTLittleWorldSessionViewController")];
        if (littleWorldSession) {
            [updateMsgs enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                
                if ([obj isKindOfClass:[NIMMessage class]]) {
                    NIMMessage *message = (NIMMessage *)obj;
                    BOOL notifyMessage = message.messageType == NIMMessageTypeNotification;
                    if (notifyMessage) {
                            NIMNotificationObject *object = message.messageObject;
                            NIMTeamNotificationContent *content = (NIMTeamNotificationContent *)object.content;
                            if (object.notificationType == NIMNotificationTypeTeam && content.operationType == NIMTeamOperationTypeLeave) {
                                //移除
                                NIMMessageModel *model = [[NIMMessageModel alloc] initWithMessage:message];
                                [self.dataSource deleteMessageModel:model];
                                [updateMsgs removeObject:obj];
                            }
                    }
        ...
        if (updateMsgs.count) {
            ...
            [wself.dataSource checkAttachmentState:updateMsgs];
        }
        
        if (handler) {
            handler(updateMsgs, error);
        }
    }];
}
    ...
    if (![self.sessionConfig respondsToSelector:@selector(autoFetchWhenOpenSession)]
        ...
        [self.dataSource resetMessages:^(NSError *error) {
            if([weakSelf.delegate respondsToSelector:@selector(didFetchMessageData)])
            {
                ...
                [weakSelf findTeamMemberLeaveMessages];
                [weakSelf.delegate didFetchMessageData];
            }
        }];
    }
        ...
        switch (type) {
            case PHAssetMediaTypeImage:
            {
                for (UIImage *image in images) {
                    ...
                    // 用于统计小世界用户在小世界群聊中发送图片的活跃度
                   [GetCore(LittleWorldCore) reportWorldMemberoActiveType:1 reportUid:GetCore(AuthCore).getUid.userIDValue worldId:GetCore(LittleWorldCore).reportWorldID];
                }
                if (path) {
                    NIMMessage *message;
...
- (void)onViewWillAppear
{
    ...
    // 这里延迟0.2秒。是为了避免进入私聊页面，当聊天记录有图片时，会闪一下又恢复
...
//群成员离开
- (void)findTeamMemberLeaveMessages {
    [self.dataSource.items enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[NIMMessageModel class]]) {
            NIMMessageModel *model = (NIMMessageModel *)obj;
