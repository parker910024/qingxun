...
NSString *const kSSCustomKeychainErrorDomain = @"com.samsoffes.sskeychain";

NSString *const kSSCustomKeychainAccountKey = @"acct";
NSString *const kSSCustomKeychainCreatedAtKey = @"cdat";
NSString *const kSSCustomKeychainClassKey = @"labl";
NSString *const kSSCustomKeychainDescriptionKey = @"desc";
NSString *const kSSCustomKeychainLabelKey = @"labl";
NSString *const kSSCustomKeychainLastModifiedKey = @"mdat";
NSString *const kSSCustomKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef SSCustomKeychainAccessibilityType = NULL;
#endif

@interface SSCustomKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation SSCustomKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = SSCustomKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kSSCustomKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (SSCustomKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return SSCustomKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (SSCustomKeychainAccessibilityType) {
		CFRelease(SSCustomKeychainAccessibilityType);
	}
	SSCustomKeychainAccessibilityType = accessibilityType;
}
#endif
