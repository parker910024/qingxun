...
@interface XCKeyFrameAnimationView ()
...
@implementation XCKeyFrameAnimationView
        ...
        if (bImgIndex != self.currentFrameIndex) {
            ...
            if (strongActor) {
                ...
                dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
                    CGImageRef copyImageRef = [frame xc_decodedCGImageRefCopy];
...
- (void)imageIndexAtTime:(NSTimeInterval)time
    ...
    if (timeDiff < repeatsDuration) {
        ...
        if (returnInfo) {
            returnInfo(bIsLastRepeat,reversedIndex);
        }
...
- (void)frameIndexAtTime:(NSTimeInterval)time
    ...
    if (returnInfo) {
        returnInfo(repeat >= _animationRepeatCount-1,frameIndex);
    }
}
