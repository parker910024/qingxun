...
@interface NIMInputView()<NIMInputToolBarDelegate,NIMInputEmoticonProtocol,NIMContactSelectDelegate,UserCoreClient>
{
...
- (void)setup
{
    if (!_toolBar)
    {
        ...
        if ((![self.session.sessionId isEqualToString:secretaryUid] && ![self.session.sessionId isEqualToString:systemUid]) &&
            GetCore(ImPublicChatroomCore).privateChatLevelNo && GetCore(ImPublicChatroomCore).privateChatLevelNo >0 && GetCore(ImPublicChatroomCore).privateChatLevelNo > _myuserInfo.userLevelVo.experLevelSeq.integerValue) {
            [_toolBar setEnable:NO];
            [_toolBar setText:[NSString stringWithFormat:@"用户等级不足%lld级，不可以发言哦~", GetCore(ImPublicChatroomCore).privateChatLevelNo]];
        }
...
- (void)setMyuserInfo:(UserInfo *)myuserInfo {
    ...
    NSString *systemUid = keyWithType(KeyType_SystemNotifyUid, devMode);
    if ((![self.session.sessionId isEqualToString:secretaryUid] && ![self.session.sessionId isEqualToString:systemUid]) &&
        _myuserInfo.userLevelVo.experLevelSeq.integerValue < GetCore(ImPublicChatroomCore).privateChatLevelNo) {
        [self.toolBar setEnable:NO];
        [self.toolBar setText:[NSString stringWithFormat:@"用户等级不足%lld级，不可以发言哦~", GetCore(ImPublicChatroomCore).privateChatLevelNo]];
    }else {
